...
template<unsigned SIZE> CKMC1DbReader<SIZE>::CKMC1DbReader(const CKmerFileHeader& header, const CInputDesc& desc, CPercentProgress& percent_progress, KmerDBOpenMode open_mode) :
	...
	min_prefix(desc.min_prefix),
	max_prefix(desc.max_prefix)
{
	...
	prefix_left_to_read = (1 << header.lut_prefix_len * 2);
	...
	open_files();
	total_kmers = total_kmers_left;
	// progress_id = percent_progress.RegisterItem(total_kmers);

	uint64 suffix_left_to_read = total_kmers_left * record_size;
...
template<unsigned SIZE> void CKMC1DbReader<SIZE>::reload_pref_buff()
{
	...
	if (to_read == 0)
	{
		prefix_buff[0] = header.total_kmers - num_to_skip_back;//guard
		return;
	}
	...
	{
		prefix_buff[to_read] = header.total_kmers - num_to_skip_back;//guard
	}
	kmers_left_for_current_prefix = prefix_buff[0] - num_to_skip_front;
}
...
template<unsigned SIZE> void CKMC1DbReader<SIZE>::open_files()
{
	...
	suffix_fd = open(suffix_file_name.c_str(), O_RDONLY);
	suffix_file = fdopen(suffix_fd, "rb");
	...
	prefix_fd = open(prefix_file_name.c_str(), O_RDONLY);
	prefix_file = fdopen(prefix_fd, "rb");
	...
	num_to_skip_back = 0;
	my_fseek(prefix_file, min_prefix * sizeof(uint64) + 4, SEEK_SET);
	fread((char*)&num_to_skip_front, sizeof(num_to_skip_front), 1, prefix_file);

	if (max_prefix > 0 && max_prefix < prefix_left_to_read) {
		my_fseek(prefix_file, max_prefix * sizeof(uint64) + 4, SEEK_SET);
		uint64 last_num;
		fread((char*)&last_num, sizeof(last_num), 1, prefix_file);
		num_to_skip_back = total_kmers - last_num;
	}
	uint64 num_to_skip = num_to_skip_back + num_to_skip_front;

	if (total_kmers_left < num_to_skip) {
		std::cerr << "Error: failed to skip to correct position in file" << "\n";
		exit(1);
	}

    prefix_left_to_read = max_prefix - min_prefix;
    if (!min_prefix) {
		my_fseek(prefix_file, sizeof(uint64) + 4, SEEK_SET);
		--prefix_left_to_read;
    } else {
		my_fseek(prefix_file, min_prefix * sizeof(uint64) + 4, SEEK_SET);
    }

    if (prefix_left_to_read && feof(prefix_file)) {
    	std::cerr << "Error: prematurely reached the end of the prefix file" << "\n";
    	exit(1);
    }
	...
	my_fseek(suffix_file, num_to_skip_front * record_size, SEEK_CUR); //skip KMCS

	total_kmers_left -= num_to_skip;

	if (total_kmers_left && feof(suffix_file)) {
    	std::cerr << "Error: prematurely reached the end of the suffix file" << "\n";
    	exit(1);
    }
	std::cout << "reading " << total_kmers_left << " kmers from " << prefix_file_name << " with fds " << prefix_fd << " " << suffix_fd << std::endl;
}
...
#define FILL_BUNDLE_LOOP_UNROLL_CODE 														      \
	...
	uint64 tmp = prefix_buff[prefix_buff_pos] - num_to_skip_front;  						      \
	++prefix_buff_pos;									   									      \
	if (prefix_buff_pos >= prefix_buff_size)												      \
	reload_pref_buff();																		      \
																							      \
	local_kmers_left_for_current_prefix = prefix_buff[prefix_buff_pos] - tmp - num_to_skip_front; \
	}																						      \
																							      \
