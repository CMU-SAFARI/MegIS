...
template<unsigned SIZE> class CTools
{
	...
	bool simple_set()
	{
		std::cout << "prefix length for 1st: " << config.headers[0].lut_prefix_len << "\n";
		std::cout << "prefix length for 2nd: " << config.headers[1].lut_prefix_len << "\n";
		if (config.headers[0].lut_prefix_len == config.headers[1].lut_prefix_len
				&& config.simple_output_desc.size() == 1) {
			uint64 prefix_len = config.headers[0].lut_prefix_len;
			uint64 max_prefix = (1llu << (prefix_len * 2));
			uint64 block_size = max_prefix / config.avaiable_threads;
			std::cout << "num threads: " << config.avaiable_threads << "\n";

			#pragma omp parallel for num_threads(config.avaiable_threads)
			for (uint64 min_prefix = 0; min_prefix < max_prefix; min_prefix += block_size) {
				...
				auto in_desc0 = config.input_desc[0];
				auto in_desc1 = config.input_desc[1];
				in_desc0.min_prefix = min_prefix;
				in_desc1.min_prefix = min_prefix;
				in_desc0.max_prefix = local_max_prefix;
				in_desc1.max_prefix = local_max_prefix;

				auto out_desc = config.simple_output_desc[0];
				out_desc.file_src += std::to_string(min_prefix) + "-" + std::to_string(local_max_prefix);

				auto header0 = config.headers[0];
				auto header1 = config.headers[1];

				CInput<SIZE> *db1, *db2;

				db1 = db_reader_factory<SIZE>(header0, in_desc0, KmerDBOpenMode::sorted);
				db2 = db_reader_factory<SIZE>(header1, in_desc1, KmerDBOpenMode::sorted);

				CBundle<SIZE> input1(db1), input2(db2);

				CDbWriter<SIZE>* writer = db_writer_factory<SIZE>(out_desc);
				writer->MultiOptputInit();

				vector<COutputBundle<SIZE>*> output_bundles;
				output_bundles.push_back(new COutputBundle<SIZE>(out_desc.op_type, out_desc.counter_op, *writer));
